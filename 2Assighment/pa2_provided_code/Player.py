...
class Player:
    ...
    def alphaBetaMove(self, board, ply):
        ...
        for m in board.legalMoves(self):
            ...
            if again:
                s = self.maxValuePruned(nb, ply-1, turn, -INFINITY, INFINITY)
            else:
    ...
    def maxValuePruned(self, board, ply, turn, alpha, beta):
        ...
        for m in board.legalMoves(self):
            ...
            if again:
                s = self.maxValuePruned(nextBoard, ply - 1, turn, alp, bet)
            else:
    ...
    def minValuePruned(self, board, ply, turn, alpha, beta):
        ...
        for m in board.legalMoves(self):
            ...
            again = nextBoard.makeMove(self, m)
            if again:
                s = self.minValuePruned(nextBoard, ply - 1, turn, alp, bet)
            else:
    ...
    def custom(self, board):
        ...
        ply = 14 - (sum(cupOfThis) + sum(cupOfOppo)) / 10
        ...
        for m in board.legalMoves(self):
            ...
            opp = Player(self.opp, self.type, self.ply)
            if again:
                s = self.customMax(nb, ply-1, turn, -INFINITY, INFINITY)
            else:
                s = opp.customMin(nb, ply-1, turn, -INFINITY, INFINITY)
            #and see what the opponent would do next
            if s > score:
                #if the result is better than our best score so far, save that move,score
                move = m
    ...
    def customMax(self, board, ply, turn, alpha, beta):
        ...
        for m in board.legalMoves(self):
            ...
            again = nextBoard.makeMove(self, m)
            if again:
                s = self.customMax(nextBoard, ply - 1, turn, alp, bet)
            else:
                s = opponent.customMin(nextBoard, ply-1, turn, alp, bet)
    ...
    def customMin(self, board, ply, turn, alpha, beta):
        ...
        for m in board.legalMoves(self):
            ...
            again = nextBoard.makeMove(self, m)
            if again:
                s = self.customMin(nextBoard, ply - 1, turn, alp, bet)
            else:
                s = opponent.customMax(nextBoard, ply-1, turn, alp, bet)
            #print "s in minValue is: " + str(s)
            score = min(s, score)
            if alp >= score:
                return score
    ...
    def chooseMove(self, board):
        ...
        if self.type == self.HUMAN:
            ...
            val, move, player = self.alphaBetaMove(board, self.ply)
...
class jlt587(Player):
    ...
    def score1(self, board):
        ...
        return scorediff #+ numdiff #+ zeroDiff
