...
class HMM:
	...
	def trainEmissions( self, trainingData, trainingLabels ):
		...
		for i in xrange(len(trainingData)):
			...
			for j in xrange(len(oneSketchFeatures)):
		...
		for s in featureVals.keys():
			for f in featureVals[s].keys():
				...
				if self.featuresCorD[f] == DISCRETE:
					...
					for i in xrange(len(self.emissions[s][f])):
	...
	def label( self, data ):
		...
		for x in self.states:
			...
			for y in self.states:
				former[x][y] = 0
		# Compute the Partial Probability of the firat state
		
		for state in self.states:
			a = self.priors[state] * self.getEmissionProb(state, data[0])          
			former[state][state] = a
		...
		for idx, x in enumerate(data):
			if idx == 0:
				...
				temp = {}
				for state in self.states:
					# Compute the prob given evidence
					evi = self.getEmissionProb(state, x)
					# Initialize reversed transitions
					paths = {}
					# Compute PP from each state
					for pstate in self.states:
						s = max(formers[-1][pstate].iteritems()
							, key=operator.itemgetter(1))[0]
						paths[pstate] = self.transitions[pstate][state] * \
										formers[-1][pstate][s] * evi
						
					temp[state] = paths
				formers.append(copy.deepcopy(temp))
		# Find the path backward
		for idx, state in reversed(list(enumerate(formers))):
			if idx == len(ret) - 1:
				temp = {}
				for s in self.states:
					maximizer = max(state[s].iteritems(),
							 key=operator.itemgetter(1))[0]
					ma = state[s][maximizer]
					temp[s] = ma

				ret[idx] = max(temp.iteritems(),
							 key=operator.itemgetter(1))[0]
				
				ps = max(state[ret[idx]].iteritems(),
							 key=operator.itemgetter(1))[0]
...
class StrokeLabeler:
	def __init__(self):
		...
		self.featureNames = ['length', 'curv', 'area']
		
		self.contOrDisc = {'length': CONTINUOUS, 'curv': CONTINUOUS, 'area': CONTINUOUS}
	...
	def featurefy( self, strokes ):
		''' Converts the list of strokes into a list of feature dictionaries
			...
			passed into the HMM'''
		ret = []
		# print "Stroke length is: " + str(len(strokes))
		for s in strokes:
	...
	def confusion(self, trueLabels, classifications):
		...
		for idx, x in enumerate(classifications):
			if trueLabels[idx] == 'drawing':
				if x == trueLabels[idx]:
					ret['drawing']['drawing'] += 1
				else:
					ret['drawing']['text'] += 1
			elif trueLabels[idx] == 'text':
				if x == trueLabels[idx]:
					ret['text']['text'] += 1
				else:
					ret['text']['drawing'] += 1
...
class Stroke:
	...
	def boundedArea(self):
		''' Returns the bounded area of the stroke '''
		left = self.points[0][0]
		right = self.points[0][0]
		up = self.points[0][1]
		down = self.points[0][1]
		for p in self.points[1:]:
			left = min(left, p[0])
			right = max(right, p[0])
			up = min(up, p[1])
...
# 				Big[x][y] = 0
# 	sum = 0
# 	for labelfile in IFileList:
# 		labelfile = "../trainingFiles/0128_1.7.1.labeled.xml"
# 		cl = sl.labelFile(labelfile, "result.txt")
# 		strokes, labels = sl.loadLabeledFile(labelfile)
# 		# print '\nReal labels is: \n' + str(labels)
# 		confusion_matrix = sl.confusion(labels, cl)
# 		for x in sl.labels:
# 			for y in sl.labels:
# 				Big[x][y] += confusion_matrix[x][y]

# 		sum += confusion_matrix['text']['text'] + confusion_matrix['drawing']['drawing'] \
# 		+ confusion_matrix['text']['drawing'] + confusion_matrix['drawing']['text']
# 	print Big
	...
	# numVals = [4]
	# hm = HMM(states, features, contOrDisc, numVals)
	# hm.isTrained = True
	# # All the probabilities start uninitialized until training
	# # features = {"Dry": ,"Dryish": ,"Damp": ,"Soggy": }
	# hm.priors = {"S": 0.63, "C": 0.17, "R":0.20}
	# hm.emissions = {
	...
	# "R": {"S": 0.125, "C": 0.675, "R":0.375}} #transition model
	# data = [
	# {"H": 0}, 
	# {"H": 2}, 
	# {"H": 3}]
